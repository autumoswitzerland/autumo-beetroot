...
public class BeanProcessor {
	 ...
	 */
	public Entity toBean(final ResultSet rs, final Class<?> type) throws SQLException {
	 ...
	 */
	public List<Entity> toBeanList(final ResultSet rs, final Class<?> type) throws SQLException {
	 ...
	 */
	private Entity createBean(final ResultSet rs, final Class<?> type, final PropertyDescriptor[] props,
			final int[] columnToProperty) throws SQLException {

		final Entity entity = (Entity) this.newInstance(type);
		return populateBean(rs, entity, props, columnToProperty);
	}
	 ...
	 * @throws SQLException if a database error occurs.
	 */
	public Entity populateBean(final ResultSet rs, final Entity entity) throws SQLException {
		final PropertyDescriptor[] props = this.propertyDescriptors(entity.getClass());
		final ResultSetMetaData rsmd = rs.getMetaData();
		final int[] columnToProperty = this.mapColumnsToProperties(rsmd, props);
		return populateBean(rs, entity, props, columnToProperty);
	}
	 ...
	 * @throws SQLException if a database error occurs.
	 */
	private Entity populateBean(final ResultSet rs, final Entity entity, final PropertyDescriptor[] props, final int[] columnToProperty) throws SQLException {
		for (int i = 1; i < columnToProperty.length; i++) {
			...
			this.callSetter(entity, prop, value);
		}
		return entity;
	}
	 ...
	 * {@code PropertyDescriptor[]} for the bean property that matches the column
		...
		for (int col = 1; col <= cols; col++) {
			...
			if (columnName.contains("_")) {
				...
				for (int p = 0; p < parts.length; p++) {
					String cp = parts[p];
					if (p > 0)
						propertyName += cp.substring(0, 1).toUpperCase() + cp.substring(1, cp.length());
					else
						propertyName += cp;
				}
			...
			for (int i = 0; i < props.length; i++) {
				...
				// We could process annotations here...

				/*
				 * @autumo: We don not process annotations here! final Column column =
				 * prop.getReadMethod().getAnnotation(Column.class); String propertyColumnName =
				 * null; if (column != null) { propertyColumnName = column.name(); if
				 * (propertyColumnName.contains("_")) { final String parts[] =
				 * propertyColumnName.split("_"); propertyColumnName = ""; for (int p = 0; p <
				 * parts.length; p++) { String cp = parts[p]; if (p > 0) propertyColumnName +=
				 * cp.substring(0, 1).toUpperCase() + cp.substring(1, cp.length()) ; else
				 * propertyColumnName += cp; } } } else { propertyColumnName = prop.getName(); }
				 */

				String propertyColumnName = prop.getName();
